...
## Taiwan Crawler — TFDA All Drug Product Approval Data Set

`app/crawlers/taiwan/crawler_tw_1.py` ingests [data.gov.tw dataset 9122](https://data.gov.tw/en/datasets/9122), TFDA's full drug-product approvals export. The dataset's "Visit this site" link is `https://data.fda.gov.tw/data/opendata/export/36/json`; despite the `/json` path, that endpoint actually returns a ZIP archive (confirmed live) containing one JSON member with a flat array of every approved drug product, in Traditional Chinese.

The license number (`許可證字號`) is not unique per row — one license appears once per manufacturing site (raw-material plant, packaging plant, etc). The crawler groups all rows sharing a license number and folds the manufacturer-specific fields (name, address, country, process) into a `manufacturers` array on one saved record, keyed by license number. It is public, requires no credentials, and has no attached PDF/label file to download.
